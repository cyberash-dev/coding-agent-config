...
install_codex_review_home() {
  ...
  # A review is a bounded, read-only pass over a diff: it needs neither the
  # operator's reasoning depth nor a browser, and a sub-agent fan-out would
  # multiply the run by its own count. The model is left to codex's own default:
  # what a review costs is turns, and a cheaper model that needs more of them to
  # reach the same finding is not cheaper. CODEX_REVIEW_MODEL pins one per
  # machine where the account says otherwise.
