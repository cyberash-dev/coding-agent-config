...
description: Full reference for Spec-Driven Development on brownfield code. Defines 17 typed normative templates (Behavior, Invariant, Contract, Scenario, NFR, Constraint, Policy, Migration, Delta, GeneratedArtifact, ExternalDependency, LocalizationContract, Surface, Partition, Implementation binding, Open-Q, ASSUMPTION), three-gate lifecycle (baseline-valid / spec-valid / implementation-valid), partition-scoped semver per Surface with cascade from Policy/Invariant, separation of Migration spec-lifecycle from runtime-state, Discovery scope and iterative debt budget, full stop-condition list, and the Template Requiredness Matrix. Use whenever the user asks to write, draft, author, review, audit, update, or evolve a specification; whenever they say "the spec is the source of truth", "spec-first", "spec-driven", or "specification before code"; whenever they ask which template fits a requirement (Contract vs Invariant vs Policy vs Migration), how to version a public API, how to describe a third-party integration, how to scope a brownfield migration, or how to set up the three CI gates; whenever an agent is about to do recon on existing code and produce a Brownfield baseline, or is judging whether a spec is ready to hand off to a code-gen agent; whenever the user asks "what belongs in the spec vs what does the agent decide", "should this be in the spec", "is this spec ready", "how do I document this in the spec", "I need to spec out X". Examples of trigger phrases - "write a spec for X", "draft a specification", "review my spec", "is this spec valid", "spec out the new feature", "напиши спеку", "составь спецификацию", "спека первой", "написать ТЗ", "описать в спеке". The condensed daily rule lives at rules/spec-driven-development.md; this skill is the full reference with all field tables, conditional triggers, cycle-closing rationale, and the v3 recommendation on temporal-layer consolidation.
trigger: /spec-driven-development
---
